...
export class LocalDocumentStorage implements FileStorage{
 ...
 private path(key:string){if(!/^tenants\/[0-9a-f-]{36}\/documents\/[0-9a-f-]{36}\/versions\/[0-9a-f-]{36}\/[^/]+$/i.test(key))throw new Error("Invalid storage key");const path=resolve(this.root,key);if(!path.startsWith(resolve(this.root)+"/"))throw new Error("Invalid storage path");return path;}
