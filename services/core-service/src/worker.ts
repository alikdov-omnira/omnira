...
const pool=new Pool({connectionString:databaseUrl}),worker=new NotificationWorker(pool,{batchSize:Number(process.env.NOTIFICATION_WORKER_BATCH_SIZE??25),maxAttempts:Number(process.env.NOTIFICATION_WORKER_MAX_ATTEMPTS??5),dueSoonDays:Number(process.env.NOTIFICATION_DUE_SOON_DAYS??3)}),ocrProvider=new TesseractOcrProvider(),ocrRunner=new OcrJobRunner(pool,new OcrService(pool,ocrProvider),ocrEnvironmentInteger("OCR_WORKER_BATCH_SIZE",1,1,4)),analysisRunner=new DocumentAnalysisRunner(pool,new DocumentAnalysisService(new DocumentAnalysisRepository(pool),new DeterministicDocumentClassifier()),analysisEnvInt("ANALYSIS_WORKER_BATCH_SIZE",1,1,4)),interval=Math.max(1000,Number(process.env.NOTIFICATION_WORKER_POLL_MS??5000));
