...
export function OmniroCommandCenter({session,navigate,forceRendererFailure=false}:{session:Session;navigate:Navigate;forceRendererFailure?:boolean}){
 const initialEnvironment=parseEnvironmentRoute(typeof location==="undefined"?"":location.hash),[data,setData]=useState<OmniroLoadedData>(),[error,setError]=useState<unknown>(),[quality,setQuality]=useState<OmniroQuality>(defaultQuality),[environment,setEnvironment]=useState(initialEnvironment),[environmentProjectId,setEnvironmentProjectId]=useState<string|undefined>(initialEnvironment?.projectId),[interaction,dispatch]=useReducer(reduceInteraction,parseOmniroRoute(typeof location==="undefined"?"#omniro":location.hash),initialInteraction);
 const load=()=>{setError(undefined);void loadOmniroData(session).then(setData).catch(setError)};
 useEffect(load,[session]);
 useEffect(()=>{const restore=()=>{const route=parseEnvironmentRoute(location.hash);setEnvironment(route);setEnvironmentProjectId(route?.projectId);dispatch({type:"ROUTE",route:parseOmniroRoute(location.hash)})};addEventListener("hashchange",restore);addEventListener("popstate",restore);return()=>{removeEventListener("hashchange",restore);removeEventListener("popstate",restore)}},[]);
 ...
 const goLayer=(layer:OmniroEnvironmentLayer)=>{const route={layer,roleId:layer==="role"?"director":undefined,projectId:layer==="role"?project?.id:undefined};history.pushState({omniro:true},"",`${location.pathname}${location.search}${environmentHash(route)}`);setEnvironment(route);dispatch({type:"ROUTE",route:{level:"operational"}})};
 const goRole=(roleId:string)=>{const route={layer:"role"as const,roleId,projectId:project?.id};history.pushState({omniro:true},"",`${location.pathname}${location.search}${environmentHash(route)}`);setEnvironment(route);dispatch({type:"ROUTE",route:{level:"operational"}})};
 const selectEnvironmentProject=(projectId:string)=>{setEnvironmentProjectId(projectId);if(environment?.layer==="role"){const route={...environment,projectId};history.pushState({omniro:true},"",`${location.pathname}${location.search}${environmentHash(route)}`);setEnvironment(route)}};
 ...
 if(environment)return <Suspense fallback={<section className="oc-load"><LoadingState label="Loading OMNIRO operating environment"/></section>}><OmniroOperatingEnvironment session={session} projects={data.projects} project={project} runtime={runtime} runtimes={runtimes} directorSources={data.director} flows={flows} layer={environment.layer} roleId={environment.roleId} onLayer={goLayer} onRole={goRole} onProject={selectEnvironmentProject} onModule={(id,projectId=project.id)=>go({level:"focus",projectId,moduleId:id})} onWorkspace={openWorkspace} onContext={(page,id)=>navigate(page,id)}/></Suspense>;
