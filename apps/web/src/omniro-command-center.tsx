...
import type{OmniroModuleRuntimeAdapter,OmniroRuntimeInput}from"./omniro-module-contract.js";
...
type Navigate=(page:"omniro"|"overview"|"construction-projects"|"project-workspace"|"documents"|"tasks",projectId?:string,tab?:WorkspaceTab)=>void;
...
export function OmniroCommandCenter({session,navigate,forceRendererFailure=false}:{session:Session;navigate:Navigate;forceRendererFailure?:boolean}){
 ...
 const openWorkspace=(id:string)=>{const item=buildingModuleRegistry.get(id);if(item?.workspace&&runtime.modules.get(id)?.availability!=="unavailable")go({level:"workspace",projectId:project.id,moduleId:id,entityId:runtime.modules.get(id)?.record?.entityId})};
 if(environment)return <Suspense fallback={<section className="oc-load"><LoadingState label="Loading OMNIRO operating environment"/></section>}><OmniroOperatingEnvironment session={session} projects={data.projects} project={project} runtime={runtime} flows={flows} layer={environment.layer} onLayer={goLayer} onProject={setEnvironmentProjectId} onModule={id=>go({level:"focus",projectId:project.id,moduleId:id})} onWorkspace={openWorkspace}/></Suspense>;
