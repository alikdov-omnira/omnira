...
export function OmniroCommandCenter({session,navigate,forceRendererFailure=false}:{session:Session;navigate:Navigate;forceRendererFailure?:boolean}){
 ...
 const goRole=(roleId:string)=>{const route={layer:"role"as const,roleId};history.pushState({omniro:true},"",`${location.pathname}${location.search}${environmentHash(route)}`);setEnvironment(route);dispatch({type:"ROUTE",route:{level:"operational"}})};
 ...
 if(environment)return <Suspense fallback={<section className="oc-load"><LoadingState label="Loading OMNIRO operating environment"/></section>}><OmniroOperatingEnvironment session={session} projects={data.projects} project={project} runtime={runtime} runtimes={runtimes} directorSources={data.director} flows={flows} layer={environment.layer} roleId={environment.roleId} onLayer={goLayer} onRole={goRole} onProject={setEnvironmentProjectId} onModule={(id,projectId=project.id)=>go({level:"focus",projectId,moduleId:id})} onWorkspace={openWorkspace} onContext={(page,id)=>navigate(page,id)}/></Suspense>;
