...
const stageRules:Array<[RegExp,OmniroStage,string]>=[[/room scanner|room scan|сканер комнат|скан помещения|сканирование помещений/,"room-scan","Room Scanner"],[/technical assignment|техническое задание|тз\b/,"technical-assignment","Technical Assignment"],[/design project|дизайн проект|дизайн-проект/,"design-project","Design Project"]];
export class LocalOrchestratorInterpreter implements OrchestratorInterpreter{
 async interpret(input:string,context:OmniroContext):Promise<OrchestratorInterpretation>{
  ...
  if(/^(show projects|projects|pokaż projekty|projekty|покажи проекты|покажи все проекты|покажи состояние компании|проекты)$/.test(n))return result(n,"show_projects","show-projects","Showing all authoritative projects.");
  ...
  if(/return to company view|company view|вернуться к компании|верни общий вид|вернись к компании|обзор компании/.test(n))return result(n,"company_view","company-view","Returning to company view.");
  ...
  const open=n.match(/^(?:open project|show project|focus on|открой проект|покажи проект|сфокусируйся на)\s+(.+)$/);
  if(open){
