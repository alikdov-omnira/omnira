...
import{api,type DesignProjectDto,type EngineeringNormDto,type RoomScanDto,type Session,type TechnicalAssignmentDto,type TechnologyDto,type WorkScopeDto}from"./api.js";
...
export async function loadOmniroData(session:Session):Promise<OmniroLoadedData>{
 ...
 const canScope=hasPermission(session,"work_scopes.read"),canNorm=hasPermission(session,"engineering_norms.read");
 const[scanPage,assignments,designs,technologies,scopeList,normList]=await Promise.all([
