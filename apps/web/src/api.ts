...
export type CommercialEstimateListDto={id:string;projectId:string;code:string;title:string;revisionNumber:number;status:CommercialEstimateStatus;mode:"budget"|"preliminary"|"tender"|"approved_client"|"internal_cost";currency:string;totals:CommercialEstimateTotalsDto;version:number};
export type CommercialEstimateLineDto={id:string;work_code:string;localized_description:string;target_element:string;quantity:number;unit_code:string;labor_cost:number;material_cost:number;equipment_cost:number;other_cost:number;overhead:number;internal_cost:number;unit_price:number;net_total:number;discount_amount:number;vat_amount:number;gross_total:number;profit:number;margin_percent:number;currency:string;price_source:string;calculation_trace:Record<string,unknown>;engineering_references:{workScopeSnapshotId:string;normSnapshotId:string;knowledgeVersionId:string;technologyVersionId:string;materialConsumptionSnapshotId:string;materialConsumptionFingerprint:string;regionalPricingSnapshotId:string;regionalPricingFingerprint:string;normFingerprint:string};norm_revision_number:number;technology_version_id:string};
export type CommercialEstimateAnalysisDto={estimateId:string;status:CommercialEstimateStatus;complete:boolean;profitability:{profit:number|null;marginPercent:number|null;negativeProfit:boolean};sourceSnapshots:{materialConsumptionSnapshotId:string;pricingSnapshotId:string};recommendations:Array<{code:string;recommendation:string}>;mutatedData:false;approvedDataChanged:false};
export type CommercialEstimateDto=CommercialEstimateListDto&{revisionId:string;lines:CommercialEstimateLineDto[];analysis?:CommercialEstimateAnalysisDto;approvedSnapshot?:ApprovedSnapshotDto};
