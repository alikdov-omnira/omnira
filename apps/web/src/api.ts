...
export type MaterialConsumptionListDto={id:string;projectId:string;code:string;title:string;revisionNumber:number;status:"draft"|"review_required"|"ready_for_approval"|"approved"|"superseded"|"cancelled";version:number};
export type MaterialConsumptionWastePolicy={engineeringWastePercent:number;cuttingWastePercent:number;packagingWastePercent:number;transportReservePercent:number;projectReservePercent:number;regionalReservePercent:number;customerReservePercent:number;ruleCode:string;ruleVersion:string};
export type MaterialConsumptionLineDto={id:string;requirementCode:string;engineeringRequirements:Record<string,unknown>;environmentalRequirements:Record<string,unknown>;formula:Record<string,unknown>;normRevisionNumber:number;technologyVersionId:string;netQuantity:number;unitCode:string;materialId?:string;selectionReason?:string;substitutesMaterialId?:string;selectionState:"unselected"|"selected"|"substitution_selected";packageType?:string;packageSize?:number;packageUnitCode?:string;roundingRuleCode?:string;roundingRuleVersion?:string;wastePolicy:Partial<MaterialConsumptionWastePolicy>;wasteQuantity:number;reserveQuantity:number;totalQuantity:number;packageCount:number;packageRemainder:number;availableQuantity:number;remainingQuantity:number;availabilitySourceReference?:string;version:number};
export type MaterialConsumptionDto=MaterialConsumptionListDto&{workScopeSnapshotId:string;normSnapshotId:string;knowledgeVersionId:string;technologyVersionId:string;workItemId:string;evaluatedParameters:Record<string,unknown>;workQuantity:number|null;workQuantityUnit:string|null;revisionId:string;lines:MaterialConsumptionLineDto[];approvedSnapshot?:ApprovedSnapshotDto};
export type MaterialConsumptionAnalysisDto={runId:string;status:string;materialReady:boolean;missingRequirements:string[];wasteQuantity:number;reserveQuantity:number;recommendations:Array<{code:string;requirementCode:string;recommendation:string}>;availabilityRisks:Array<{lineId:string;remainingQuantity:number;unitCode:string}>;mutatedData:false;priceCalculationPerformed:false};
...
export type RegionalPricingStatus="draft"|"review_required"|"ready_for_approval"|"approved"|"superseded"|"cancelled";
export type RegionalPricingListDto={id:string;code:string;title:string;revisionNumber:number;status:RegionalPricingStatus;currency:string;countryCode:string;regionCode:string;effectiveFrom:string;effectiveTo?:string;version:number};
export type RegionalPriceEntryDto={id:string;priceKind:"labor"|"material"|"equipment"|"subcontractor"|"transport"|"disposal";referenceCode:string;description:string;rateType:string;unitCode:string;unitPrice:number;priceVariant:string;minimumOrderQuantity?:number;packagePrice?:number;palletPrice?:number;validFrom:string;validTo?:string;sourceReference:string;metadata:Record<string,unknown>};
export type RegionalPricingRuleDto={code:string;percent:number;condition?:string};
export type RegionalPricingDto=RegionalPricingListDto&{revisionId:string;city?:string;customerCategory:string;commercialProfile:string;priceSource:string;overheadRules:RegionalPricingRuleDto[];taxRule?:RegionalPricingRuleDto&{mode?:string};marginRules:RegionalPricingRuleDto[];discountRules:RegionalPricingRuleDto[];adjustmentRules:RegionalPricingRuleDto[];calculationVersion:string;entries:RegionalPriceEntryDto[];approvedSnapshot?:ApprovedSnapshotDto};
export type CompanyPriceBookStatus="draft"|"review_required"|"ready_for_approval"|"approved"|"superseded"|"cancelled";
export type CompanyPriceBookEntryDto={id:string;entryKind:"labor"|"material";workItemId:string|null;materialId:string|null;measurementUnitId:string;unitCode:string;internalCost:number;sellingPrice:number;currency:string;effectiveFrom:string;effectiveTo:string|null;sourceReference:string;notes:string|null;version:number};
export type CompanyPriceBookListDto={id:string;code:string;title:string;purpose:string|null;revisionId:string;revisionNumber:number;status:CompanyPriceBookStatus;currency:string;effectiveFrom:string;effectiveTo:string|null;version:number};
export type CompanyPriceBookDto=CompanyPriceBookListDto&{entries:CompanyPriceBookEntryDto[];approvedSnapshot?:ApprovedSnapshotDto};
export type CompanyPriceResolutionDto={internalCost:number;sellingPrice:number;marginAmount:number;marginPercent:number|null;priceBookId:string;revisionId:string;revisionNumber:number;priceBookSnapshotId:string;fingerprint:string;entryId:string;entryKind:"labor"|"material";workItemId:string|null;materialId:string|null;measurementUnitId:string;unitCode:string;currency:string;sourceReference:string;effectiveOn:string;resolutionVersion:"company-price-resolution-v1"};
export type CompanyPriceRegionalComparisonDto={available:false;reason:"NO_COMPATIBLE_REGIONAL_REFERENCE"|"AMBIGUOUS_REGIONAL_REFERENCE";mutated:false}|{available:true;companySellingPrice:number;regionalReference:number;companyInternalCost:number;marginAmount:number;marginPercent:number|null;regionalVariance:number;regionalVariancePercent:number|null;currency:string;unitCode:string;companySnapshotId:string;companyFingerprint:string;regionalSnapshotId:string;regionalFingerprint:string;mutated:false};
export type CommercialEstimateStatus="draft"|"review_required"|"ready_for_approval"|"approved"|"superseded"|"cancelled";
export type CommercialEstimateTotalsDto={laborCost:number;materialCost:number;equipmentCost:number;otherCost:number;overhead:number;internalCost:number;unitPrice:number;netTotal:number;discountAmount:number;vatAmount:number;grossTotal:number;profit:number;marginPercent:number;currency:string;trace?:Record<string,unknown>};
export type CommercialEstimatePricingAuthority="regional_pricing_v1"|"company_price_book_v1";
export type CommercialEstimateCompanyPriceBookAuthority={priceBookId:string;companyWorkItemId?:string;code?:string;title?:string;revisionId:string;revisionNumber:number;snapshotId:string;fingerprint:string};
export type CommercialEstimateListDto={id:string;projectId:string;code:string;title:string;revisionNumber:number;status:CommercialEstimateStatus;mode:"budget"|"preliminary"|"tender"|"approved_client"|"internal_cost";currency:string;totals:CommercialEstimateTotalsDto;version:number;calculationVersion?:string;pricingAuthority?:CommercialEstimatePricingAuthority;companyPriceBookId?:string;companyWorkItemId?:string;companyPriceBookRevisionId?:string;companyPriceBookRevisionNumber?:number;companyPriceBookSnapshotId?:string;companyPriceBookFingerprint?:string};
export type CommercialEstimateLineDto={id:string;work_code:string;localized_description:string;target_element:string;quantity:number;unit_code:string;labor_cost:number;material_cost:number;equipment_cost:number;other_cost:number;overhead:number;internal_cost:number;unit_price:number;net_total:number;discount_amount:number;vat_amount:number;gross_total:number;profit:number;margin_percent:number;currency:string;price_source:string;calculation_trace:Record<string,unknown>;pricing_provenance?:Record<string,unknown>;engineering_references:{workScopeSnapshotId:string;normSnapshotId:string;knowledgeVersionId:string;technologyVersionId:string;materialConsumptionSnapshotId:string;materialConsumptionFingerprint:string;regionalPricingSnapshotId:string;regionalPricingFingerprint:string;regionalPricingRole?:"comparison_only"|"pricing_authority";companyPriceBookId?:string;companyPriceBookRevisionId?:string;companyPriceBookRevisionNumber?:number;companyPriceBookSnapshotId?:string;companyPriceBookFingerprint?:string;normFingerprint:string};norm_revision_number:number;technology_version_id:string};
export type CommercialEstimateAnalysisDto={estimateId:string;status:CommercialEstimateStatus;complete:boolean;pricingAuthority?:CommercialEstimatePricingAuthority;profitability:{profit:number|null;marginPercent:number|null;negativeProfit:boolean};sourceSnapshots:{materialConsumptionSnapshotId:string;pricingSnapshotId:string;regionalPricingRole?:"comparison_only"|"pricing_authority";companyPriceBookId?:string;companyWorkItemId?:string;companyPriceBookRevisionId?:string;companyPriceBookRevisionNumber?:number;companyPriceBookSnapshotId?:string;companyPriceBookFingerprint?:string};recommendations:Array<{code:string;recommendation:string}>;mutatedData:false;approvedDataChanged:false};
export type CommercialEstimateDto=CommercialEstimateListDto&{revisionId:string;calculationVersion?:string;pricingAuthority?:CommercialEstimatePricingAuthority;companyPriceBookAuthority?:CommercialEstimateCompanyPriceBookAuthority;regionalPricingReference?:{snapshotId:string;role:"comparison_only"|"pricing_authority"};lines:CommercialEstimateLineDto[];analysis?:CommercialEstimateAnalysisDto;approvedSnapshot?:ApprovedSnapshotDto};
...
export const api={
  ...
  materialConsumptions:(projectId?:string)=>raw<MaterialConsumptionListDto[]>(`/material-consumptions${projectId?`?projectId=${encodeURIComponent(projectId)}`:""}`),
  materialConsumption:(id:string)=>raw<MaterialConsumptionDto>(`/material-consumptions/${id}`),
  materialConsumptionAnalysis:(id:string)=>raw<MaterialConsumptionAnalysisDto>(`/material-consumptions/${id}/analysis`),
  selectConsumptionMaterial:(runId:string,lineId:string,body:{materialId:string;selectionReason:string;substitutesMaterialId?:string;wastePolicy:MaterialConsumptionWastePolicy;availableQuantity:number;availabilitySourceReference?:string;expectedVersion:number;expectedLineVersion:number})=>raw<{id:string;lineId:string;version:number;lineVersion:number}>(`/material-consumptions/${runId}/lines/${lineId}/select-material`,json("POST",body)),
  materialConsumptionReadiness:(id:string,expectedVersion:number)=>raw<{ready:boolean;issues:Array<{code:string;lineId?:string;requirementCode?:string}>}>(`/material-consumptions/${id}/readiness`,json("POST",{expectedVersion})),
  materialConsumptionApprove:(id:string,expectedVersion:number)=>raw<ApprovedSnapshotDto>(`/material-consumptions/${id}/approve`,json("POST",{expectedVersion})),
  materialConsumptionSnapshot:(id:string)=>raw<ApprovedSnapshotDto>(`/material-consumptions/${id}/approved-snapshot`),
  regionalPricings:()=>raw<RegionalPricingListDto[]>("/regional-pricing"),
  regionalPricing:(id:string)=>raw<RegionalPricingDto>(`/regional-pricing/${id}`),
  regionalPricingReview:(id:string,expectedVersion:number)=>raw<{ready:boolean;issues:Array<{code:string;kind?:string}>;version:number}>(`/regional-pricing/${id}/review`,json("POST",{expectedVersion})),
  regionalPricingApprove:(id:string,expectedVersion:number)=>raw<ApprovedSnapshotDto>(`/regional-pricing/${id}/approve`,json("POST",{expectedVersion})),
  regionalPricingSnapshot:(id:string)=>raw<ApprovedSnapshotDto>(`/regional-pricing/${id}/approved-snapshot`),
  companyPriceBooks:()=>raw<CompanyPriceBookListDto[]>("/company-price-books"),
  companyPriceBook:(id:string)=>raw<CompanyPriceBookDto>(`/company-price-books/${id}`),
  createCompanyPriceBook:(body:{code:string;title:string;purpose?:string;currency:string;effectiveFrom:string;effectiveTo?:string})=>raw<CompanyPriceBookDto>("/company-price-books",json("POST",body)),
  addCompanyPriceEntry:(id:string,body:{entryKind:"labor"|"material";workItemId?:string;materialId?:string;measurementUnitId:string;internalCost:number;sellingPrice:number;currency:string;effectiveFrom:string;effectiveTo?:string;sourceReference:string;notes?:string;expectedVersion:number})=>raw<CompanyPriceBookDto>(`/company-price-books/${id}/entries`,json("POST",body)),
  updateCompanyPriceEntry:(id:string,entryId:string,body:{expectedVersion:number;measurementUnitId?:string;internalCost?:number;sellingPrice?:number;currency?:string;effectiveFrom?:string;effectiveTo?:string|null;sourceReference?:string;notes?:string|null})=>raw<CompanyPriceBookDto>(`/company-price-books/${id}/entries/${entryId}`,json("PATCH",body)),
  companyPriceBookReview:(id:string,expectedVersion:number)=>raw<{ready:boolean;issues:Array<{code:string;entryId?:string}>;version:number}>(`/company-price-books/${id}/review`,json("POST",{expectedVersion})),
  companyPriceBookApprove:(id:string,expectedVersion:number)=>raw<ApprovedSnapshotDto>(`/company-price-books/${id}/approve`,json("POST",{expectedVersion})),
  companyPriceBookRevision:(id:string,expectedVersion:number)=>raw<CompanyPriceBookDto>(`/company-price-books/${id}/revisions`,json("POST",{expectedVersion})),
  companyPriceBookCancel:(id:string,expectedVersion:number)=>raw<{id:string;status:"cancelled";version:number}>(`/company-price-books/${id}/cancel`,json("POST",{expectedVersion})),
  companyPriceBookSnapshot:(id:string)=>raw<ApprovedSnapshotDto>(`/company-price-books/${id}/approved-snapshot`),
  resolveCompanyPrice:(id:string,body:{snapshotId:string;entryKind:"labor"|"material";workItemId?:string;materialId?:string;measurementUnitId:string;currency:string;effectiveOn:string})=>raw<CompanyPriceResolutionDto>(`/company-price-books/${id}/resolve`,json("POST",body)),
  compareCompanyPriceRegional:(id:string,body:{snapshotId:string;entryId:string;regionalPricingSnapshotId:string;effectiveOn:string})=>raw<CompanyPriceRegionalComparisonDto>(`/company-price-books/${id}/regional-comparison`,json("POST",body)),
  commercialEstimates:(projectId?:string)=>raw<CommercialEstimateListDto[]>(`/commercial-estimates${projectId?`?projectId=${encodeURIComponent(projectId)}`:""}`),
  commercialEstimate:(id:string)=>raw<CommercialEstimateDto>(`/commercial-estimates/${id}`),
  commercialEstimateAnalysis:(id:string)=>raw<CommercialEstimateAnalysisDto>(`/commercial-estimates/${id}/analysis`),
  commercialEstimateReview:(id:string,expectedVersion:number)=>raw<{ready:boolean;issues:Array<{code:string}>;version:number}>(`/commercial-estimates/${id}/review`,json("POST",{expectedVersion})),
  commercialEstimateApprove:(id:string,expectedVersion:number)=>raw<ApprovedSnapshotDto>(`/commercial-estimates/${id}/approve`,json("POST",{expectedVersion})),
  commercialEstimateSnapshot:(id:string)=>raw<ApprovedSnapshotDto>(`/commercial-estimates/${id}/approved-snapshot`),
